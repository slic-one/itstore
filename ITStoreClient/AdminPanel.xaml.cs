...
namespace ITStoreClient
{
    ...
    public partial class AdminPanel : Window
    {
        ...
        public AdminPanel()
        {
            ...
            totalSupplyLabel.Content = "";
            totalSaleLabel.Content = "";
            totalProfitLabel.Content = "";
        }
        ...
        private void searchReportClick(object sender, RoutedEventArgs e)
        {
            ...
            if (fromDate == null && toDate == null)
                sales = data.Sales.Select(x => x);

            if (fromDate != null && toDate == null)
                sales = data.Sales.Where(s => s.DateSale >= fromDate).Select(x => x);

            if (fromDate == null && toDate != null)
                sales = data.Sales.Where(s => s.DateSale <= toDate).Select(x => x);
            ...
            if (user != null && customer != null)
                ...
                var rez = from p in data.Products
                          ...
                          select new
                          {
                              ...
                              TotalSalePrice = (decimal)(p.Price + p.Price * markup.Percent / 100) * pr.Quantity,
                              Profit = (decimal)(pr.Quantity * p.Price * markup.Percent / 100)
                          };
                ...
                foreach (var el in resultList)
                {
                    reportList.Add(new Report(el.Name, el.id, el.Quantity, el.Price, el.Measurement, el.TotalSupplyPrice, el.TotalSalePrice, el.Profit));
                }
                ...
                dataGridReport.IsReadOnly = true;
            }
        ...
        private void loadReport_Click(object sender, RoutedEventArgs e)
        {
            ...
            openFileDialog1.InitialDirectory = Directory.GetCurrentDirectory() + "\\reports";
            if (openFileDialog1.ShowDialog() == true)
            {
                if ((myStream = openFileDialog1.OpenFile()) != null)
                {
                    try
                    {
                        XmlSerializer serializer = new XmlSerializer(typeof(List<Report>), new XmlRootAttribute("Reports"));
                        reportList = (List<Report>)serializer.Deserialize(myStream);
                        dataGridReport.ItemsSource = reportList;
                        dataGridReport.IsReadOnly = true;
                        myStream.Close();
                        totalSupplyLabel.Content = "";
                        totalSaleLabel.Content = "";
                        ...
                        totalSale = 0;
                        totalProfit = 0;
                        foreach (var el in reportList)
                        {
                            totalSupply += el.TotalSuplyPrice;
                            totalSale += el.TotalSalePrice;
                            totalProfit += el.Profit;
                        }
...

        {
            ...
            saveFileDialog1.InitialDirectory = Directory.GetCurrentDirectory() + "\\reports";
            if (dateFrom.SelectedDate != null && dateTo.SelectedDate != null)
                saveFileDialog1.FileName = dateFrom.SelectedDate.Value.ToShortDateString() + "_" + dateTo.SelectedDate.Value.ToShortDateString();

            if (cashierName != null) {
                saveFileDialog1.FileName += "_cashier_" + cashierName;
            }
            if (customerName != null)
            {
                saveFileDialog1.FileName += "_customer_" + customerName;
            }
            ...
            if (saveFileDialog1.ShowDialog() == true)
            {
                if ((myStream = saveFileDialog1.OpenFile()) != null)
                {
                    var serializer = new XmlSerializer(typeof(List<Report>),
                                    new XmlRootAttribute("Reports"));
                    serializer.Serialize(myStream, reportList);
                    myStream.Close();
                }
            }
        }
